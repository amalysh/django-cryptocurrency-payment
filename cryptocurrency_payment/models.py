...
def create_new_payment(
    ...
    fiat_amount = float(fiat_amount) + max(float(fee_fix), float(fiat_amount) * (float(fee_pct) / 100.0))
    ...
    if not address:
        ...
        derivation_path = get_backend_config(crypto, key="DERIVATION_PATH")
        address = backend_obj.generate_new_address(address_generated_count, address_type=address_type, derivation_path=derivation_path)
